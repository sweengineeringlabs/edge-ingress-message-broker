...
/// Errors returned by [`MessageConsumer`](crate::MessageConsumer) operations.
#[derive(Debug, thiserror::Error)]
pub enum ConsumerError {
    /// Failed to subscribe to the given topic.
    #[error("subscribe failed on topic '{topic}': {reason}")]
    Subscribe {
        /// The topic name that the subscription was attempted on.
        topic: String,
        /// The reason the subscription failed.
        reason: String,
    },
